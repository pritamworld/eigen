...
describe(@"getShowFeedItems", ^{
    ...
    it(@"gets items", ^{
        ...
        Fair *fair = [Fair modelWithJSON:@{ @"id" : @"fair-id-3",
                                            @"name" : @"The Armory Show",
                                            @"organizer" : @{@"profile_id" : @"fair-profile-id"} }];
...
it(@"getPosts", ^{
    ...
    it(@"returns feed timeline", ^{
        ...
        waitUntil(^(DoneCallback done) {
            ...
            Fair *fair = [Fair modelWithJSON:@{ @"id" : @"fair-id-4",
                                                @"name" : @"The Armory Show",
                                                @"organizer" : @{@"profile_id" : @"fair-profile-id"} }];
            ARFairNetworkModel *networkModel = [[ARFairNetworkModel alloc] init];
...
describe(@"getOrderedSets", ^{
    before(^{
        [OHHTTPStubs stubJSONResponseAtPath:@"/api/v1/sets" withResponse:@[
            @{ @"id" : @"52ded6edc9dc24bccc0000a4",
               @"key" : @"curator",
               @"name" : @"Highlights from Art World Influencers",
               @"item_type" : @"FeaturedLink" },
            @{ @"id" : @"52ded6edc9dc24bccc0000a5",
               @"key" : @"curator",
               @"name" : @"Other Highlights",
               @"item_type" : @"FeaturedLink" },
            @{ @"id" : @"52ded6edc9dc24bccc0000b5",
               @"key" : @"something else",
               @"name" : @"Something Else",
               @"item_type" : @"FeaturedLink" }
        ]];
    ...
    it(@"returns ordered sets by key", ^{
        waitUntil(^(DoneCallback done) {
            ...
            Fair *fair = [Fair modelWithJSON:@{ @"id" : @"fair-id-5",
                                                @"name" : @"The Armory Show",
                                                @"organizer" : @{@"profile_id" : @"fair-profile-id"} }];
            ARFairNetworkModel *networkModel = [[ARFairNetworkModel alloc] init];
...
describe(@"getMapInfo", ^{
    ...
    it(@"returns maps for fair", ^{
        Fair *fair = [Fair modelWithJSON:@{ @"id" : @"fair-id-6",
                                            @"name" : @"The Armory Show",
                                            @"organizer" : @{@"profile_id" : @"fair-profile-id"} }];
        ARFairNetworkModel *networkModel = [[ARFairNetworkModel alloc] init];
